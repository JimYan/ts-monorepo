...
type GeneratorOptions = Protobuf.IParseOptions &
...
function getImportPath(
  to: Protobuf.Type | Protobuf.Enum | Protobuf.Service
): string {
...
function getRelativeImportPath(
  from: Protobuf.Type | Protobuf.Service,
  to: Protobuf.Type | Protobuf.Enum | Protobuf.Service
) {
  return getPathToRoot(from) + getImportPath(to);
}

function getTypeInterfaceName(
  type: Protobuf.Type | Protobuf.Enum | Protobuf.Service
) {
...
function getImportLine(
  ...
  if (dependency.parent instanceof Protobuf.Type) {
    ...
    if (
      ...
      importedTypes = `${inputName(dependency.name)} as ${inputName(
        typeInterfaceName
      )}, ${outputName(dependency.name)} as ${outputName(typeInterfaceName)}`;
    } else if (dependency instanceof Protobuf.Service) {
...
function formatComment(
  ...
  comment?: string | null,
  options?: Protobuf.ReflectionObject["options"]
) {
...
function getTypeNamePermissive(
  fieldType: string,
...
function getFieldTypePermissive(
  field: Protobuf.FieldBase,
  options: GeneratorOptions
): string {
  const valueType = getTypeNamePermissive(
    field.type,
    field.resolvedType,
    field.repeated,
    field.map,
...
function generatePermissiveMessageInterface(
  formatter: TextFormatter,
...
function getTypeNameRestricted(
  fieldType: string,
  resolvedType: Protobuf.Type | Protobuf.Enum | null,
  repeated: boolean,
  map: boolean,
...
function generateRestrictedMessageInterface(
  formatter: TextFormatter,
  messageType: Protobuf.Type,
  options: GeneratorOptions,
  nameOverride?: string
...
function generateMessageInterfaces(
  ...
  for (const childType of childTypes.sort(compareName)) {
    ...
    if (childType instanceof Protobuf.Type) {
      generatePermissiveMessageInterface(
        formatter,
        childType,
        options,
        nameOverride
      ...
      formatter.writeLine("");
      generateRestrictedMessageInterface(
        formatter,
        childType,
        options,
        nameOverride
...
function generateEnumInterface(
  ...
  for (const key of Object.keys(enumType.values)) {
    if (options.includeComments) {
      formatComment(
        formatter,
        enumType.comments[key],
        (enumType.valuesOptions ?? {})[key]
      );
    }
    formatter.writeLine(
      `${key}: ${options.enums == String ? `'${key}'` : enumType.values[key]},`
    );
  }
  formatter.unindent();
  ...
  formatter.writeLine(
    `export type ${outputName(name)} = typeof ${name}[keyof typeof ${name}]`
  );
}
...
function generateNestClientModuleProxy(
  formatter: TextFormatter,
...
function generateServiceClientInterface(
  ...
  formatter.writeLine(
    `export interface ${serviceType.name}Client extends grpc.Client {`
  );
  formatter.indent();
  for (const methodName of Object.keys(serviceType.methods).sort()) {
    ...
    for (const name of [methodName, camelCase(methodName)]) {
      ...
      const requestType = inputName(
        getTypeInterfaceName(method.resolvedRequestType!)
      );
      const responseType = outputName(
      ...
      if (method.requestStream) {
        if (method.responseStream) {
          ...
          formatter.writeLine(
            `${name}(metadata: grpc.Metadata, options?: grpc.CallOptions): ${callType};`
          );
          formatter.writeLine(
            `${name}(options?: grpc.CallOptions): ${callType};`
          );
        } else {
          // Client streaming
          const callType = `grpc.ClientWritableStream<${requestType}>`;
          formatter.writeLine(
            `${name}(metadata: grpc.Metadata, options: grpc.CallOptions, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(metadata: grpc.Metadata, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(options: grpc.CallOptions, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(callback: ${callbackType}): ${callType};`
          );
        }
      } else {
        if (method.responseStream) {
          ...
          formatter.writeLine(
            `${name}(argument: ${requestType}, metadata: grpc.Metadata, options?: grpc.CallOptions): ${callType};`
          );
          formatter.writeLine(
            `${name}(argument: ${requestType}, options?: grpc.CallOptions): ${callType};`
          );
        } else {
          // Unary
          const callType = "grpc.ClientUnaryCall";
          formatter.writeLine(
            `${name}(argument: ${requestType}, metadata: grpc.Metadata, options: grpc.CallOptions, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(argument: ${requestType}, metadata: grpc.Metadata, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(argument: ${requestType}, options: grpc.CallOptions, callback: ${callbackType}): ${callType};`
          );
          formatter.writeLine(
            `${name}(argument: ${requestType}, callback: ${callbackType}): ${callType};`
          );
...
function generateServiceHandlerInterface(
  formatter: TextFormatter,
  serviceType: Protobuf.Service,
  options: GeneratorOptions
) {
  ...
  formatter.writeLine(
    `export interface ${serviceType.name}Handlers extends grpc.UntypedServiceImplementation {`
  );
  formatter.indent();
  for (const methodName of Object.keys(serviceType.methods).sort()) {
    const method = serviceType.methods[methodName];
    ...
    const requestType = outputName(
      getTypeInterfaceName(method.resolvedRequestType!)
    );
    const responseType = inputName(
      getTypeInterfaceName(method.resolvedResponseType!)
    );
    if (method.requestStream) {
      if (method.responseStream) {
        // Bidi streaming
        formatter.writeLine(
          `${methodName}: grpc.handleBidiStreamingCall<${requestType}, ${responseType}>;`
        );
      } else {
        // Client streaming
        formatter.writeLine(
          `${methodName}: grpc.handleClientStreamingCall<${requestType}, ${responseType}>;`
        );
      }
    } else {
      if (method.responseStream) {
        // Server streaming
        formatter.writeLine(
          `${methodName}: grpc.handleServerStreamingCall<${requestType}, ${responseType}>;`
        );
      } else {
        // Unary
        formatter.writeLine(
          `${methodName}: grpc.handleUnaryCall<${requestType}, ${responseType}>;`
        );
      }
    }
    formatter.writeLine("");
...
function generateServiceDefinitionInterface(
  formatter: TextFormatter,
  serviceType: Protobuf.Service,
  options: GeneratorOptions
) {
  const { inputName, outputName } = useNameFmter(options);
  formatter.writeLine(
    `export interface ${serviceType.name}Definition extends grpc.ServiceDefinition {`
  );
  formatter.indent();
  for (const methodName of Object.keys(serviceType.methods).sort()) {
    const method = serviceType.methods[methodName];
    ...
    formatter.writeLine(
      `${methodName}: MethodDefinition<${inputName(requestType)}, ${inputName(
        responseType
      )}, ${outputName(requestType)}, ${outputName(responseType)}>`
    );
  }
  formatter.unindent();
  formatter.writeLine("}");
...
function generateServiceInterfaces(
  ...
  formatter.writeLine(
    `// Original file: ${(serviceType.filename ?? "null")?.replace(/\\/g, "/")}`
  );
  formatter.writeLine("");
...
function containsDefinition(
  definitionType: typeof Protobuf.Type | typeof Protobuf.Enum,
  namespace: Protobuf.NamespaceBase
): boolean {
...
function generateDefinitionImports(
  formatter: TextFormatter,
  namespace: Protobuf.NamespaceBase,
  options: GeneratorOptions
) {
  ...
  if (imports.length) {
    formatter.writeLine(
      `import type { ${imports.join(", ")} } from '@grpc/proto-loader';`
    );
  }
}
...
function generateSingleLoadedDefinitionType(
  formatter: TextFormatter,
  nested: Protobuf.ReflectionObject,
  options: GeneratorOptions
) {
  if (nested instanceof Protobuf.Service) {
    if (options.includeComments) {
    ...
    formatter.writeLine(
      `${nested.name}: SubtypeConstructor<typeof grpc.Client, ${typeInterfaceName}Client> & { service: ${typeInterfaceName}Definition }`
    );
...
function generateLoadedDefinitionTypes(
  formatter: TextFormatter,
  namespace: Protobuf.NamespaceBase,
  options: GeneratorOptions
) {
...
function generateFilesForNamespace(
  namespace: Protobuf.NamespaceBase,
  options: GeneratorOptions
): Promise<void>[] {
  ...
  for (const nested of namespace.nestedArray) {
    ...
    if (nested instanceof Protobuf.Type) {
      ...
      if (options.verbose) {
        console.log(
          `Writing ${options.outDir}/${getPath(nested)} from file ${
            nested.filename
          }`
        );
      }
      ...
      const ClientProxyFile =
        stripLeadingPeriod(nested.fullName).replace(/\./g, "/") + "Module.ts";
      ...
      filePromises.push(
        writeFile(
          `${options.outDir}/${ClientProxyFile}`,
          fileFormatter2.getFullText()
        )
      );
      ...
      const proxyfile =
        stripLeadingPeriod(nested.fullName).replace(/\./g, "/") +
        "ClientStub.ts";
      filePromises.push(
        writeFile(
          `${options.outDir}/${proxyfile}`,
          fileFormatter3.getFullText()
        )
      );
    } else if (isNamespaceBase(nested)) {
...
function writeFilesForRoot(
  ...
  filePromises.push(
    writeFile(
      `${options.outDir}/${masterFileName}`,
      masterFileFormatter.getFullText()
    )
  );
...
async function runScript() {
  ...
  const argv = yargs
    ...
    .describe({
      ...
      longs:
        "The type that should be used to output 64 bit integer values. Can be String, Number",
      enums:
        "The type that should be used to output enum fields. Can be String",
      bytes:
        "The type that should be used to output bytes fields. Can be String, Array",
      defaults: "Output default values for omitted fields",
      arrays:
        "Output default values for omitted repeated fields even if --defaults is not set",
      objects:
        "Output default values for omitted message fields even if --defaults is not set",
      ...
      includeComments:
        "Generate doc comments from comments in the original files",
      ...
      inputBranded:
        'Output property for branded type for  "permissive" types with fullName of the Message as its value',
      outputBranded:
        'Output property for branded type for  "restricted" types with fullName of the Message as its value',
    })
    .demandOption(["outDir", "grpcLib"])
    .demand(1)
    .usage("$0 [options] filenames...")
    .epilogue(
      "WARNING: This tool is in alpha. The CLI and generated code are subject to change"
    ).argv;
  if ((argv as unknown as any).verbose) {
    console.log("Parsed arguments:", argv);
  }
  addCommonProtos();
  writeAllFiles((argv as unknown as any)._ as string[], {
